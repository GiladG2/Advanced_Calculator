from exceptions import (
    DecimalOfDecimalException,
    EmptyParenthesesException,
    FactorialException,
    IncorrectHashtagException,
    InvalidCharacterException,
    TildeException,
    UnopenedParenthesesException,
)
from Queue import Queue
from Stack import Stack
from Tokens import is_binary_op, is_left_associative, is_op, is_unary_op, is_valid_token
...
def is_negation(expression, i: int):
    ...
    return i == 0 or (
        (
            ...
            and (is_binary_op(prev_token) or prev_token == "(" or prev_token is "~")
        )
...
def is_valid_tilde(stack, expression, i):
    return (
        stack.is_empty()
        or (stack.top().value is not "u" and stack.top().value is not "w")
    ) and not prev_is_numeric(expression, i)


# returns if both tokens are unary op
...
# returns True if current should be inserted to the Queue,
# False if not.
def compare_op(current, expression, i):
    return (
        ...
        and (
            both_unary(expression[i], current.value)
            or is_binary_op(expression[i])
        )
    )
...
# recieves the next token and determines if there is an incorrect use
# of an unary op
def incorrect_unary(next_token):
    return next_token.isdigit() or (
        not is_unary_op(next_token)
    ...
    while i < len(expression):
        ...
        if not expression[i].isnumeric():
            if expression[i] is ")":
                ...
                continue
            elif check_enqueue_binary_op(current, expression, i) and compare_op(
                ...
                if current.value == "u" or current.value is "w":
                    ...
                    while (
                        not stack.is_empty()
                        and is_op(stack.top().value)
                        and compare_op(stack.top(), expression, i)
                    ):
                        queue.enqueue(stack.pop())
            if expression[i] is "-" and is_negation(expression, i):
